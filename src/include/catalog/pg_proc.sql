...
-- #define MPP_EXECUTION_SEGMENT_TYPE 23
 ...
 CREATE FUNCTION gp_endpoints_info(OUT token text, OUT cursorname text, OUT sessionid int4, OUT hostname text, OUT port int4, OUT dbid int4, OUT userid oid, OUT status text) RETURNS SETOF record LANGUAGE internal VOLATILE EXECUTE ON MASTER AS 'gp_endpoints_info' WITH (OID=7178, DESCRIPTION="mpp endpoints information");

 CREATE FUNCTION gp_endpoints_status_info(OUT token text, OUT databaseid int4, OUT senderpid int4, OUT receiverpid int4, OUT attach_status int4, OUT dbid int4, OUT sessionid int4, OUT userid oid) RETURNS SETOF record LANGUAGE internal VOLATILE EXECUTE ON ANY AS 'gp_endpoints_status_info' WITH (OID=7179, DESCRIPTION="endpoints status information");
